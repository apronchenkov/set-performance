...
// Sequential insert
template <class IntegerSet> Duration SequentialInsertTestImpl(IntegerSet set, int n) {
  auto start = Now();
  for (int i = 0; i < n; ++i) {
    set.insert(i);
  }
...
template <class IntegerSet> Duration SequentialInsertTest(IntegerSet set, int n, int k = 32) {
...
int main() {
  for (int n = 256; n <= 1024 * 1024; n += n) {
      ...
      std::cout << n << '\t'
                << SequentialInsertTest(std::move(btree_set), n).count() << '\t'
